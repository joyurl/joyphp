...
/*
	...
	作用：根据请求参数，调用对应对象获取内容，并把内容转换为HTML代码
...
class _views
{
	...
	public function _views(){
		...
		if(!empty($_menu)) $this->_hiddenstr=$this->_hiddenstr.'<input type="hidden" name="'.urlencode($_menu).'" value="'.urlencode($_menu).'">';
	}
	...
	public function deal($obj){
		...
		//获得菜单
		if(method_exists($this->_obj,'getMenu')){
			$this->_menu=call_user_func(array($this->_obj,'getMenu'));
		}

		$this->_cmd=$this->_para['_cmd'];
		if(empty($this->_cmd)){
			$this->_cmd=$this->_menu['dataconf']['cmd'];
		}
		...
		if($ret['result']>0){
			if(empty($ret['pagetitle'])){
				foreach($this->_menu['data'] as $arr){
		...
		//处理返回数据（正常的返回）
			...
			if(!empty($ret['msg'])){
				...
				$ret['html']=$this->view_list($ret);
			}
			else if($ret['type']=='form'){
				$ret['html']=$this->view_form($ret);
			}
		}
		return $ret;
	}
	...
	public function sent(){
		...
		if(method_exists($this->_obj,$this->_cmd)){
			$ret=call_user_func(array($this->_obj,$this->_cmd));
		}
		else{
			$ret=Array('result'=>1,'msg'=>'Undefined method \''.$this->_cmd.'\'!');
		}
		...
		if(isset($ret['type']) and $ret['type']=='save'){
			...
			//失败（jumpcmd使用原来的cmd）
				...
				if($this->_obj->_isAPI) usleep(500000);
				$ret=call_user_func(array($this->_obj,$this->_cmd));
			}
		}
	...
	protected function view_list($ret){
		...
		$html='
	<table width="100%" cellspacing="1" cellpadding="2" border="0"><tr><td height="26">';

		//显示搜索框
		if(!empty($conf['searchtitle']) and !empty($conf['searchfind'])){
			if(empty($conf['searchsubmit'])) $conf['searchsubmit']='Submit';
			$html=$html.'
			<form action="'.$this->_pagename.'" method="GET">'.$conf['searchtitle'].$this->_hiddenstr.'<input type="hidden" name="_cmd" value="'.$this->_cmd.'">';
			if(is_array($conf['searchfind'])) $html=$html.$this->get_search_box($conf['searchfind']);
			$html=$html.'<input type="text" name="_k" value="'.get_html(getpost('_k')).'">';
		...
		$html=$html.($conf['pageprev']).(!empty($form['search']) ? " 搜索结果" :'').'共'.intval($pageinfo['rowsnum']).'条记录';


		if($pageinfo['pagenum']>1){
			$html=$html.' &nbsp; 每页'.$pageinfo['pagesize'].'条 第'.$pageinfo['page'].'页/共'.$pageinfo['pagenum'].'页';
			//显示页码
			if($page>1)
		...
		$html=$html.' '.$this->encode($conf['addstring']).'</td></tr></table>
	<table width="100%" cellspacing="0" cellpadding="0" border="0"><tr><td height="8"></td></tr></table>';
		...
		//表头
		$html=$html.'<tr bgcolor="#EEEEEE" class="tbl_head" align="center">';
		$tdconf=Array();
		$n=0;
		foreach($ret['datahead'] as $arr){
			...
			$tdconf[$name]=" ".(!empty($arr['align'])?" align=\"".$arr['align']."\"":"");
			$n++;
		}
		$html=$html.'</tr>';
		...
		foreach($ret['data'] as $arr){
			...
			foreach($tdconf as $name =>$tdset){
				$showstr=$this->encode($arr[$name]);
				$html=$html.'
			<td'.(strlen($tdset)>2 ? ' '.$tdset :'').($n==0 ? ' height="'.$tdheight.'"':'').' class="tdrow">'.$showstr.'</td>';
				$n++;
			}
			$html=$html.'</tr>';
			$r++;
    ...
    <TABLE cellSpacing=1 cellPadding=3 width="100%" align=center  bgColor="#CCCCCC" class="tbl_main" border=0>
		<tr class="tbl_head" bgcolor="#EEEEEE" align="center"><td colspan="2" height="26"><b>'.$ret['pagetitle'].'</b></td></tr>';
		...
		if(!empty($ret['dataconf']['backstr']) and !empty($ret['dataconf']['backcmd'])){
			$html=$html.'
		<input type="button" value="'.get_html($ret['dataconf']['backstr']).'" onclick="window.open(\''.$this->_pagename.'&_cmd='.$ret['dataconf']['backcmd'].'\',\'_self\');">';
		}
		$html=$html.'
		</td></tr>
		</form>
...
_inputSet["'.$name.'"]=Array("'.$set['title'].'",'.$v2.','.$v1.','.$v3.');';
		...
		if($w1>0) $w2=100-$w1;
		$html='
		<tr  bgcolor="#FFFFFF" class="tbl_row"><td align="right" height="27" '.($w1>0?' width="'.$w1.'%"':'').' class="tdrow">'.($set['title']).(strtoupper($set['notnull'])=='Y' ? ' <font color="#EE0000">*</font>':'').'</td><td'.($w1>0?' width="'.$w2.'%"':'').' class="tdrow">';
		...
		if($set['type']=='view'){
			...
			$html=$html.'<input type="text" name="'.$name.'" id="'.$name.'" value="'.$value.'"'.(!empty($set['size']) ? ' maxlength="'.$size.'"' :'').($width>0 ? ' style="width:'.$width.';"' :'').$onblur.'>'.$set['unit'];
		}
		else if($set['type']=='password'){
			$html=$html.'<input type="password" name="'.$name.'" id="'.$name.'" value="'.$value.'"'.(!empty($set['size']) ? ' maxlength="'.$size.'"' :'').($width>0 ? ' style="width:'.$width.';"' :'').$onblur.'>'.$set['unit'];
		}
		...
		//显示表单后的注释
		if(!empty($set['note'])){
			$html=$html.' &nbsp; <font color="#666666">'.$set['note'].'</font>';
		}
	...
	function get_search_box($findArr){
		...
		foreach($findArr as $_value =>$arr){
			...
			$html=$html.'
			<option  value="'.$_value .'"'.(($n==0 or $_value==$_f and strlen($_f)>0)?" selected":"").'>'.$arr['title'].'&nbsp;'.$showSet[$_t].'</option>';
			$n++;
		}
		$html=$html.'</select>';
		return $html;
