...
class _views
{
	...
	public function deal($obj){
		...
		//获得菜单
		$initMenu='menuSet';
		if(method_exists($this->_obj,$initMenu)){
			$ret=call_user_func(array($this->_obj,$initMenu));
			if($ret['result']>0){
			...
			//处理错误
			$ret=Array('result'=>1,'msg'=>'Undefined method ('.$initMenu.')!');
			$this->_errMsg=$ret['msg'];
			return $ret;
		}
	...
	public function sent(){
		...
		if(method_exists($this->_obj,$this->_cmd)){
			...
			$this->_errMsg='Undefined method \''.$this->_cmd.'\'!';
			$ret=Array('result'=>4,'msg'=>$this->_errMsg);
			return $ret;
		}
