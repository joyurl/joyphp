...
/*
	...
	用于设置供TB处理的模型结构，数据保存到系统表
	
*/


if(!defined('__CLASS__TM__')){
  define('__CLASS__TM__', 1);

class TM extends TB
{
	//自身实例化
	public function self_init($DB){
		return new TM($DB);
	}
	...
	protected function getSearchSQL(&$_key,$_f,$findArr){
		...
		//@$findArr=Array();定义搜索项
		//Array("type:类型","title:显示名称","field:搜索的字段名称(有附表时为附表字段)","tbl:附表表名(真实名称)","set:(type=12)搜索设置或(type=2)第二字段(type=13或3设置字符)或(其它)主附表关联字段名")

		//type类型
		//0 == 等于（限数值型，适用于可以搜索多个值的情况，默认按等于进行搜索。支持运算符搜索：用A|B搜索两个值;用A&B搜索A到B之间的值;前加!A搜索不等于A的值;>A大于A;<A小于A）
		//1 === 绝对等于，不支持运算符搜索，适用于只需要搜索一个值的情况
		//2 =@= 双字段AND搜索（限数值型，限搜索主表内字段），不能关联附表搜索： table 值不使用，set为第二个值的搜索字段
		//3 *== 多字段OR搜索（适用于数值型，不能关联附表搜索） set格式set=Array("字段2","字段3","字段n")
		...
		//11 ≈ 单字段包含搜索(搜索内容包含输入值的记录，适用于对较长的字符进行搜索，比如文章内容、备注等) 
		//12 ≡ 智能单字段搜索，对field自定义规则模糊LIKE搜索(但不支持运算符搜索)，不能关联附表搜索。
		//        若有set设置限搜索主表且值为绝对等于，按输入值匹配set设置进行搜索， set格式为： 
		//        Array(array("/^[1-9][0-9]{0,9}\$/i","user_id",1),array("/^1[3-9][0-9]{9}\$/i","user_mobile",0));
		//13 ≌ 多字段OR搜索(LIKE)（适合于字符串，不能关联附表搜索，不支持运算符搜索）,set格式同3
			...
			if($_type==0) {
				...
				if(isset($fArr['set']) and is_array($fArr['set'])){
					foreach($fArr['set'] as $_find){
						...
						$_tmp=$_tmp." OR  {$_find} = ".$this->formatNum($_k,1);
					}
				}
				if(!empty($_tmp)){
					$_tmp=$_tmp." ($_field = ".$this->formatNum($_k,1).substr($_tmp,4).")";
				}
				//默认
				else{
					$_tmp=$_tmp." $_field = ".$this->formatNum($_k,1);
	...
	//@set[] =Array("title"=>"@表单显示内容","select"=>"表单显示值","fieldset"=>'查询字段，为空则使用field',"value"=>"对应查询值，为空时则直接使用select值进行查询，支持用|查询2-6个值");
	protected function getSortSQL($form,$sortArr){
		...
		foreach($sortArr as $conf){
			...
			$_tmp='';//当前分类框
			$_select=trim($form($conf['name']));
			foreach($conf['set'] as $sets){

				if($sets['select']!=$_select) continue;
				...
				if(!empty($sets['value'])){
					//支持搜索多个值
					if(preg_match("/\|/i",$sets['value'])){
						$tmpstr='';
						$rows=explode("|",$sets['value']);
						$n=0;
						foreach($rows as $_v){
							$_v=trim($_v);
							if(strlen($_v)>0){
					...
					$_tmp=$_tmp." = ".$this->DB->getsql($_select,$_type); //直接使用
				}
				break;
