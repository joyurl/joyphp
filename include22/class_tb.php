...
class TB
{
	...
	protected function getSearchSQL(&$_key,$_f,$findArr){
		...
		//@$findArr=Array();定义搜索项
		//Array("type:类型","title:显示名称","field:搜索的字段名称(有附表时为附表字段)","tbl:附表表名(真实名称)","set:(type=15)搜索设置或(type=2|12)第二字段(type=16设置字段)或(其它)主附表关联字段名")
		//运算符：数值(! | & > < ) 字符(! | = /A/)
		...
		//12 ＝: 双字段AND搜索（适用于字串型） 不能关联附表搜索，不支付运算符或匹配符： table 值不使用，set为第二个值的搜索字段
		...
		//16 ∽ 智能多字段OR搜索(LIKE)（适合于字符串，不能关联附表搜索，不支持运算符搜索）,set格式set=Array("字段2","字段3","字段n")
