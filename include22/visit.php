...
class _Visit
{
	...
	function _Visit($siteid=1){
		...
		//蜘蛛
		$this->_spider="(Baiduspider|Googlebot|bingbot|Yahoo.+Slurp|msnbot|Sogou.+spider|360Spider|ia_archiver|Wget|masscan|ResearchBot|WebCrawler|Scrapy|WinHttpRequest|python-requests|robot|spider|Crawler|bot.+http:)";
		//疑似蜘蛛（同时来路为空）
		$this->_spider2='(Chrome\/46\.0\.2490\.86 Safari\/537\.36|Chrome\/45\.0\.2454\.93 Safari\/537\.36|SCH-N719.+Safari\/533\.1|Firefox\/[3-4]\.0[.0-9a-z]*|Gecko\/20100101 Firefox\/[1-9][0-9]*\.0|Gecko\/20100101 Firefox\/40\.1|OPR\/11\.2\.3.+Safari\/537\.36|CPU iPhone OS [0-9]+_|MQQBrowser.+TBS)';

		//虚假referer
		$this->_refer[]='http://www.baidu.com/s?wd=www';
	...
	function vrecord($DB,$u){
		...
		//清除过期数据
			...
			if(empty($RS) or $RS['visit_clear']!=date('Y-m-d')){
				...
				$DB->query("delete from ".$this->tbl['detail']."  where visit_datetime<".$DB->getsql($deltime)." limit 50000");
				usleep(20000);
				$DB->query("Optimize table  ".$this->tbl['detail']."");
				usleep(20000);
				$DB->query("delete from ".$this->tbl['spider']."  where visit_datetime<".$DB->getsql($deltime)." limit 50000");
				usleep(20000);
